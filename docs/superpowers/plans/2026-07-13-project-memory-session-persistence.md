...
git commit -m "feat(session): use YYYYMMDD-HHMMSS-xxxx session IDs"
```
...
git commit -m "feat(context): introduce SessionHandle for rebindable session paths"
```
...
git commit -m "feat(session): add JSONL SessionTranscriptWriter"
```
...
git commit -m "feat(session): persist conversation via PersistingSessionManager"
```
...
```bash
git commit -m "feat(session): wire JSONL persistence at startup"
```
...
```bash
git commit -m "feat(prompt): load LAVENDERCODE.md with safe @include"
```
...
```bash
git commit -m "feat(prompt): assemble config, file instructions, and memory modules"
```
...
```bash
git commit -m "feat(prompt): inject file instructions into system prompt at runtime"
```
...
```bash
git commit -m "feat(session): catalog resumable sessions for /resume"
```
...
```bash
git commit -m "feat(session): restore conversations from JSONL with safeguards"
```
...
```bash
git commit -m "feat(session): cleanup expired session directories on startup"
```
...
```bash
git commit -m "feat(terminal): add /resume session picker and restore wiring"
```
...
```bash
git commit -m "feat(memory): load and truncate MEMORY.md indexes"
```
...
```bash
git commit -m "feat(memory): async note updates after agent complete"
```
...
git commit -m "docs: add implementation plan for memory and session persistence"
```
